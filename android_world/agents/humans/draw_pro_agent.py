...
class DrawPro(base_agent.EnvironmentInteractingAgent):
  ...
  def steps(self, file_name_all:str):
      file_name,type = file_name_all.split('.')
      ...
      reason = [
          "To begin creating a new drawing in Simple Draw Pro, the first step is to open the app. Using the `open_app` action is the most direct way to launch it, as we're currently on the home screen and no prior actions have been taken.",
          ' To proceed with saving the new drawing, the first step is to initiate the save process by clicking the "Save" button, which is UI element 0 on the current screen. This will likely open a dialog for specifying the file name and storage location',
          # f"""I need to type {file_name} into the editable text field (UI element 1), which is currently visible and ready for input. This aligns with the task's requirement to name the file without the extension.""",
          f"""The filename field (index 1) contains incorrect appended text. To correct this, long - pressing on the field (index 1) will likely bring up text selection options (like select all), which will allow overwriting the text with the correct name {file_name} in subsequent steps.""",
          f"""To set the correct filename for the drawing, I need to type {file_name} into the editable text field (UI element 1), which is currently visible and ready for input. This aligns with the task's requirement to name the file without the extension.""",
        f'To save the drawing in the correct format ({type}), need to select the ".{type}" radio button (UI element 6), which is currently unchecked. This step ensures the file extension matches the requirement.',
        f' To proceed with saving the drawing, after setting the correct filename and file format (.{type}), the next logical step is to confirm these settings by clicking the "OK" button (index 3). This action will likely lead to the interface for selecting the storage location (sdk_gphone_x86_64 storage area and Pictures folder), which is necessary to complete the task.',
          f' I need to click the "SAVE" button to complete the saving process of the drawing in the selected "Pictures" folder. This action will finalize the save operation for the drawing named "{file_name}".',
          'The drawing has been successfully saved, and I need to confirm that the task is complete.'
      ]
      ...
      Action: {{"action_type": "open_app", "app_name": "Simple Draw Pro"}}""",
          ...
          # f"""Reason: {reason[2]}
          #      Action: {{"action_type": "input_text", "text": "{file_name}", "index": 1}}""",
          f"""Reason: {reason[2]}
            Action: {{"action_type": "long_press", "index": 1}}""",
          f"""Reason: {reason[3]}
                Action: {{"action_type": "input_text", "text": "{file_name}", "index": 1}}""",
          f"""Reason: {reason[4]}
                Action: {{"action_type": "click", "index": {index} }}""",
          f"""Reason: {reason[5]}
