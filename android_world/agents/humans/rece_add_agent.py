...
class ReceAddAgent(base_agent.EnvironmentInteractingAgent):
  ...
  def steps(self, res):
      ...
      reason = [
          "To start adding the recipe, I need to open the Broccoli app. Since the current screen is the home screen, using the `open_app` action with app_name \"Broccoli\" is the logical first step.",
          "To add a new recipe, the logical next step is to initiate the recipe creation process. The \"New Recipe\" ImageButton (UI element 8) is clearly visible and clickable, which is the appropriate element to start creating a new recipe.",
          f"To start adding the recipe details, the first step is to input the recipe title into the \"Title\" field. UI element 6 corresponds to the \"Title\" EditText, so using the `input_text` action to enter \"{res.title}\" into this field is the logical next step",
          'To continue adding the recipe details, the next step is to input the description into the "Description" EditText (index 8). This aligns with the task of filling out all recipe fields, starting with the description provided.',
          'To access additional recipe fields like servings, preparationTime, ingredients, and directions, which are likely located below the currently visible elements, I need to scroll down the screen. This will reveal the necessary fields to continue entering the recipe details.',
           'To reveal the remaining recipe fields (servings, preparationTime, ingredients, directions) that are currently not visible, need to scroll down the screen further. This will allow accessing and entering the required details for the recipe.',
           'To access the remaining recipe fields (servings, preparationTime, ingredients, directions) that are likely hidden within the scrollable area (UI element 3, a ScrollView), scrolling down within this specific element will help reveal the necessary fields for completing the recipe entry.',
            f'To continue adding the recipe details, the next field to fill is "Servings". UI element 7 corresponds to the "Servings" EditText, so we need to input "{res.servings}" into this field using the `input_text` action.',
           f'Next, need to input the preparationTime "{res.preparationTime}" into the "Time" EditText field (index 8) to continue completing the recipe details.',
            f'Next, need to input the ingredients "{res.ingredients}" into the Ingredients EditText field (index 9) to continue completing the recipe details.',
            'Need to fill the "Directions" field (UI element 10) with the provided directions text. Using `input_text` action to enter the text directly into the specified EditText field.',
            'All required recipe details have been entered, and the "SAVE" button (index 2) is visible and clickable. Clicking it will finalize adding the recipe to the Broccoli app, completing the task.',
            'The recipe details screen confirms that all entered information (title, description, servings, preparation time, ingredients, directions) is correctly displayed, indicating the recipe has been successfully added. No further actions are needed, so the task is complete.',
      ]
      ...
      Action: {{"action_type": "open_app", "app_name": "Broccoli"}}""",
          ...
          f"""Reason: {reason[2]}
       Action: {{"action_type": "input_text", "text": "{res.title}", "index": 6}}""",
          f"""Reason: {reason[3]}
      Action: {{"action_type": "input_text", "text": "{res.description}", "index": 8}}""",
          f"""Reason: {reason[4]}
      ...
      Action: {{"action_type": "scroll", "direction": "down", "index": ""}}""",
       ...
       Action: {{"action_type": "scroll", "direction": "down", "index": 3}}""",
          f"""Reason: {reason[7]}
      Action: {{"action_type": "input_text", "text": "{res.servings}", "index": 7}}""",
          f"""Reason: {reason[8]}
      Action: {{"action_type": "input_text", "text": "{res.preparationTime}", "index": 8}}""",
          f"""Reason: {reason[9]}
      Action: {{"action_type": "input_text", "text": "{res.ingredients}", "index": 9}}""",
          f"""Reason: {reason[10]}
       Action: {{"action_type": "input_text", "text": "{res.directions}", "index": 10}}""",
          f"""Reason: {reason[11]}
