...
class ContactsAddAgent(base_agent.EnvironmentInteractingAgent):
  ...
  def steps(self, name:str,number:str):

      firstName,nextName = (name.strip().split(None, 1) + [""])[:2]
      num = number.removeprefix('+')
      reason = [
          "To create a new contact, the first step is to open the Contacts app. UI element 11 is labeled \"Contacts\" and is clickable, so clicking on it will open the app, allowing further actions to add a new contact.",
          f"""To create a new contact, the next logical step is to tap on the "Create contact" button (UI element 1) to open the contact creation interface. This aligns with the goal of adding {name}'s contact details.""",
          f""" To create a new contact for {name}, the first step is to input the first name into the "First name" field. UI element 7 corresponds to this field, so we'll use the `input_text` action to type {firstName} into it.""",
          f"""To continue creating the contact for {name}, the next step is to input the last name "{nextName}" into the "Last name" field (UI element 8), which is visible and editable on the screen. This progresses toward completing the contact details.""",
        f' To proceed with creating the contact, the phone number "{num}" needs to be entered into the "Phone" field. UI element 10 corresponds to the "Phone" editable text field, so using the `input_text` action on this index with the specified number will fulfill this step.',
        ...
        f' To correct the actual phone number, the phone number "{number}" needs to be entered into the "Phone" field.',
        f' To finalize the creation of {name}\'s contact, the "Save" button (UI element 2) needs to be clicked to store the entered details. This action will complete the task of creating the new contact.',
          f' The contact details for {name}, including the phone number {number}, are now displayed, confirming that the new contact has been successfully saved. No further actions are needed to complete the task.'
      ]
      ...
      Action: {{"action_type": "click", "index": 11}}""",
          ...
          f"""Reason: {reason[2]}
            Action: {{"action_type": "input_text", "text": "{firstName}", "index": 7}}""",
          f"""Reason: {reason[3]}
                Action: {{"action_type": "input_text", "text": "{nextName}", "index": 8}}""",
          f"""Reason: {reason[4]}
                Action: {{"action_type": "input_text", "text": "{num}", "index": 10}}""",
          f"""Reason: {reason[5]}
            Action: {{"action_type": "click", "index": 13}}""",
          f"""Reason: {reason[6]}
            Action: {{"action_type": "input_text", "text": "{number}", "index": 10}}""",
          f"""Reason: {reason[7]}
