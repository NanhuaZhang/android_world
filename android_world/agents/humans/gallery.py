...
class SaveCopyOfReceiptTaskEval(base_agent.EnvironmentInteractingAgent):
  ...
  def steps(
    ...
    file_name: str,
      ...
      reason = [
          "To start the task of copying the specified image in Simple Gallery Pro, the first step is to open the Simple Gallery Pro app. Using the `open_app` action is the most direct way to launch the app.",
          f"To initiate the copy operation for \"{file_name}\", I need to long press on the file's UI element to bring up the context menu or selection options. this element corresponds to the file's text, and long pressing it (or its associated area) is the standard way to select a file for operations like copy in gallery apps.",
          "To proceed with copying the selected file, I need to access the additional options menu. this element is labeled \"More options\" and is clickable, which likely contains the copy or duplicate functionality required for this task. Clicking on it will reveal the necessary actions to continue.",
          "To proceed with copying the selected file, I need to choose the \"Copy to\" option from the menu, which is this element. This will allow me to specify the destination folder (Download) for the file copy.",
          "To locate the Download folder (which is at the same level as DCIM), we need to access the parent directory. The \"Other folder\" button is the appropriate option to proceed to the higher-level directory where both DCIM and Download reside. Clicking this will allow us to navigate to the correct location to select the Download folder.",
          "To proceed with copying the file to the Download folder, I need to access the outermost file path to find the \"Download\" folder and click the visible \"Internal\" button.",
          "To proceed with copying the file to the Download folder, we need to select the \"Download\" folder. this element corresponds to \"Download\", which is the target folder at the same level as DCIM. Clicking on it will navigate into the Download folder, allowing us to confirm the destination by clicking \"OK\" next.",
          f"To finalize copying the file \"{file_name}\" to the Download folder, we need to confirm the destination by clicking the \"OK\" button, which is identified as this element. This action will complete the file copying process as per the task requirements.",
          "All steps to copy the file from DCIM to Download folder have been executed successfully, including selecting the file, choosing \"Copy to\", navigating to the Download folder, and confirming with \"OK\". The current screen indicates the task has been finalized, so the goal is achieved.",
      ]
      ...
      steps = [
          ...
          f"""Reason: {reason[1]}
            Action: {{"action_type": "long_press", "index": 10, "element_text": "{file_name}" }}""",
          f"""Reason: {reason[2]}
            Action: {{"action_type": "click", "index": 10, "element_text": "More options" }}""",
          f"""Reason: {reason[3]}
            Action: {{"action_type": "click", "index": 10, "element_text": "Copy to" }}""",
          f"""Reason: {reason[4]}
            Action: {{"action_type": "click", "index": 10, "element_text": "Other folder" }}""",
          f"""Reason: {reason[5]}
            Action: {{"action_type": "click", "index": 10, "element_text": "Internal" }}""",
          f"""Reason: {reason[6]}
            Action: {{"action_type": "click", "index": 10, "element_text": "Download" }}""",
          f"""Reason: {reason[7]}
