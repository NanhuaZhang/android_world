...
class ContactsDraftAgent(base_agent.EnvironmentInteractingAgent):
  ...
  def steps(self, first:str,last:str,phone:str,phone_label:str):
      phone_simple = phone.replace('-','')
      reason = [
          " To start the process of creating a new contact, I need to open the Contacts app. UI element 11 is labeled \"Contacts\" and is clickable, so clicking on it will open the Contacts app, which is the next logical step toward accessing the new contact screen.",
          f"""To proceed to the new contact screen, I need to click on the "Create contact" button, which is identified as UI element 0. This action aligns with the goal of accessing the screen where contact details can be entered.""",
          f"""  To begin entering the contact details as per the goal, start with the "First Name" field. UI element 7 corresponds to the "First name" editable text field, so use the `input_text` action to type "{first}" into this field.""",
          f""" To proceed with entering contact details, the next step is to input {last} into the Last Name field, which is identified by UI element index 8. This aligns with the task of filling out the contact form step by step.""",
        f'To continue entering contact details as per the task, the next step is to input the phone number "{phone}" into the Phone field. UI element 10 corresponds to the Phone field, so using the `input_text` action on index 10 with the specified phone number is the appropriate action.',
        f' The "Phone" field contains incorrect text，To correct this, click the delete button, which will allow delete the wrong number',
        f' To correct the actual phone number, the phone number "{phone}" needs to be entered into the "Phone" field.',
          f' To set the Phone Label to "{phone_label}", I need to interact with the spinner (UI element 11) that currently displays "Mobile". Clicking on this spinner will open the dropdown menu where I can select the "{phone_label}" label.',
          f'To set the Phone Label to "{phone_label}", we need to select the "{phone_label}" option from the open dropdown menu. UI element 18 corresponds to "{phone_label}" and is clickable, so clicking it will update the label correctly.',
          f' All required contact details (First Name, Last Name, Phone, Phone Label) have been entered correctly as specified, and the task explicitly states not to hit save. The desired state has been achieved, so the task can now be marked as complete.'
      ]
      if phone_label == 'Home':
          label_index = 18
      else:
      ...
      Action: {{"action_type": "click", "index": 11}}""",
          ...
          f"""Reason: {reason[2]}
            Action: {{"action_type": "input_text", "text": "{first}", "index": 7}}""",
          f"""Reason: {reason[3]}
                Action: {{"action_type": "input_text", "text": "{last}", "index": 8}}""",
          f"""Reason: {reason[4]}
                Action: {{"action_type": "input_text", "text": "{phone_simple}", "index": 10}}""",
          f"""Reason: {reason[5]}
            Action: {{"action_type": "click", "index": 13}}""",
          f"""Reason: {reason[6]}
            Action: {{"action_type": "input_text", "text": "{phone}", "index": 10}}""",
          f"""Reason: {reason[7]}
        Action: {{"action_type": "click", "index": 11}}""",
          f"""Reason: {reason[8]}
      Action: {{"action_type": "click", "index": {label_index}}}""",
