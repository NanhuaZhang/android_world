...
class ClockTimeAgent(base_agent.EnvironmentInteractingAgent):
  ...
  def getIndex(self, num:str):
      if num == '0':
          return 13
      return int(num) + 2

  def steps(self, hours:str,minutes:str,seconds:str):
     hours = hours.zfill(2)
     minutes = minutes.zfill(2)
     seconds = seconds.zfill(2)

     reason = [
          "To create a timer, the first step is to open the Clock app. Since the current screen is the home screen, using the `open_app` action with \"Clock\" as the app name is the appropriate next step.",
          f"""To create a timer, I first need to navigate to the Timer tab. this element corresponds to the Timer tab, which is clickable, so clicking it will take me to the Timer interface where I can set the desired time.""",
          f"""To set the timer duration, start by entering the digits for {hours} hours, {minutes} minutes, and {seconds} seconds. The first digit of the hours is '{hours[0]}', which corresponds to this element (text='{hours[0]}', clickable). Clicking this will begin inputting the time.""",
          f"""To set the timer to {hours} hours, {minutes} minutes, and {seconds} seconds, after entering "{hours[0]}" (from step 3), the next digit for the hour part is "{hours[1]}". this element corresponds to the digit "{hours[1]}", so clicking it will append "{hours[1]}" to the current input, progressing toward "{hours}" for the hours.""",
          f"""To set the timer for {hours} hours, {minutes} minutes, and {seconds} seconds, we need to input the digits \"{hours}{minutes}{seconds}\" (representing hh:mm:ss). Currently, \"{hours}\" is entered (showing as 00h 00m {hours}s). The next digit to input is \"{minutes[0]}\" (for the minutes part, starting the third digit), so click on this element (text='{minutes[0]}') to proceed building the 6-digit input.""",
          f"""The current timer input "00h 0{hours[0]}m {hours[1]}{minutes[0]}s" is invalid due to incorrect digit placement. To correct this, we need to delete the existing digits using the backspace button (this element) to start fresh and input the correct duration "{hours}{minutes}{seconds}" (representing {hours}h {minutes}m {seconds}s). Clicking the backspace will remove the last entered digit, allowing us to rebuild the correct input.""",
        f'To set the timer to {hours} hours, {minutes} minutes, and {seconds} seconds, we need to input the digits in the order {hours[0]}, {hours[1]}, {minutes[0]}, {minutes[1]}, {seconds[0]}, {seconds[1]}. After correcting previous errors, the next step is to start building the correct digit sequence by clicking on digit \'{minutes[0]}\'  to begin entering the first digit of the desired timer value.',
          f'To set the minutes value to {minutes}, after entering "{minutes[0]}" (from step 7), the next digit for the minute part is "{minutes[1]}". this element corresponds to the digit "{minutes[1]}", so clicking it will append "{minutes[1]}" to the current input, progressing toward "{minutes}" for the minutes.',
          f'To set the seconds value to {seconds}, after entering "{minutes[1]}" (from step 8), the next digit for the minute part is "{seconds[0]}". this element corresponds to the digit "{seconds[0]}", so clicking it will append "{seconds[0]}" to the current input, progressing toward "{seconds}" for the seconds.',
          f'To set the seconds value to {seconds}, after entering "{seconds[0]}" (from step 9), the next digit for the minute part is "{seconds[1]}". this element corresponds to the digit "{seconds[1]}", so clicking it will append "{seconds[1]}" to the current input, progressing toward "{seconds}" for the seconds.',
          f'The timer has created, so the task can now be marked as complete.'
         ]
     ...
     steps = [
      ...
      Action: {{"action_type": "open_app", "app_name": "Clock"}}""",
          ...
          f"""Reason: {reason[2]}
          Action: {{"action_type": "click", "index": {self.getIndex(hours[0])}}}""",
          f"""Reason: {reason[3]}
                Action: {{"action_type": "click", "index": {self.getIndex(hours[1])}}}""",
          f"""Reason: {reason[4]}
                Action: {{"action_type": "click", "index": {self.getIndex(minutes[0])}}}""",
          f"""Reason: {reason[5]}
            Action: {{"action_type": "click", "index": 14}}""",
          f"""Reason: {reason[6]}
            Action: {{"action_type": "click", "index": {self.getIndex(minutes[0])}}}""",
          f"""Reason: {reason[7]}
        Action: {{"action_type": "click", "index": {self.getIndex(minutes[1])}}}""",
          f"""Reason: {reason[8]}
      Action: {{"action_type": "click", "index": {self.getIndex(seconds[0])}}}""",
          f"""Reason: {reason[9]}
      Action: {{"action_type": "click", "index": {self.getIndex(seconds[1])}}}""",
          f"""Reason: {reason[10]}
